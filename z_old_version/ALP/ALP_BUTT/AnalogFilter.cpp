...
int AnalogFilter::write_AnalogFilter(int impn, int stpn, complex<double> pole[], complex<double> zero[])
{
	...
	if (ofs.is_open())
	{
		...
		for(int i=0; i<polem; i++)
		{
			ofs << "pole" << i << " = " << pole[i] << endl;
		}
		ofs << endl;
		for(int i=0; i<zerom; i++)
		{
			ofs << "zero" << i << " = " << zero[i] << endl;
		}
		ofs.close();
		cout << "Successfully wrote the ANALOGFILTER.TXT" << endl;
	}
	else
	{
